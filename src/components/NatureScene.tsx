...
function makeVisit(id: number): Visit {
  ...
  const scale = FAST ? 1 : rand(0.7, 1.18);
  // ~1 in 3 visits is a fly-by: the bird just crosses, no branch, no perch.
  const mode: Mode = FAST ? 'perch' : Math.random() < 0.34 ? 'flyby' : 'perch';

  if (mode === 'flyby') {
    const ltr = Math.random() < 0.5;
    const startX = ltr ? -90 : W + 90;
    const exitX = ltr ? W + 110 : -110;
    const startY = rand(0.1, 0.74) * H;
    const exitY = startY + rand(-150, 150);
    const midX = W * 0.5;
    const midY = Math.min(startY, exitY) - rand(20, 130);
    return {
      id, mode, scale, side: ltr ? 'left' : 'right',
      perchX: 0, perchY: 0, startX, startY, midX, midY, exitX, exitY, exitMidX: 0, exitMidY: 0, branchLen: 0,
    };
  }

  const side: Side = FAST ? 'left' : Math.random() < 0.5 ? 'left' : 'right';
  const perchX = FAST ? 0.3 * W : side === 'left' ? rand(0.1, 0.27) * W : rand(0.73, 0.9) * W;
  const perchY = FAST ? 0.42 * H : rand(0.24, 0.66) * H;
  const fromBelow = Math.random() < 0.5;
  const startX = side === 'left' ? -80 : W + 80;
  const startY = perchY + (fromBelow ? rand(70, 200) : rand(-200, -70));
  const midX = side === 'left' ? perchX * 0.5 : (W + perchX) / 2;
  const midY = (startY + perchY) / 2 - rand(40, 110);
  const exitX = side === 'left' ? -100 : W + 100;
  const exitY = perchY - rand(160, 330);
  const exitMidX = (perchX + exitX) / 2;
  const exitMidY = perchY - rand(50, 120);
  const branchLen = side === 'left' ? perchX + 10 : W - perchX + 10;
  return { id, mode, scale, side, perchX, perchY, startX, startY, midX, midY, exitX, exitY, exitMidX, exitMidY, branchLen };
}
...
function Bird({ visit, onDone }: { visit: Visit; onDone: () => void }) {
  ...
  const isFly = visit.mode === 'flyby';

  // hold on the branch, then leave
  useEffect(() => {
    if (phase !== 'perch') return;
    const t = setTimeout(() => setPhase('out'), rand(3500, 7000));
    return () => clearTimeout(t);
  }, [phase]);

  const flying = isFly || phase === 'in' || phase === 'out';
  // face the direction of travel; when perched, look into the page
  const faceLeft =
    isFly ? visit.exitX < visit.startX
  ...
  const target =
    phase === 'in'
      ? isFly
        ? { x: [visit.startX, visit.midX, visit.exitX], y: [visit.startY, visit.midY, visit.exitY], opacity: [0, 0.9, 0.9, 0] }
        : { x: [visit.startX, visit.midX, visit.perchX], y: [visit.startY, visit.midY, visit.perchY], opacity: [0, 0.92, 0.92] }
  ...
  const transition =
    phase === 'in' ? { duration: isFly ? rand(5.5, 8) : 3.4, ease: isFly ? ([0.45, 0, 0.55, 1] as const) : ([0.42, 0, 0.3, 1] as const) }
  ...
  return (
    <>
      {/* the branch reaches out from the edge to meet the bird, then recedes.
         Fly-bys skip it entirely — they just cross the sky. */}
      {!isFly && (
      <div
      ...
      <motion.div
        ...
        onAnimationComplete={() => {
          if (phase === 'in') { if (isFly) onDone(); else setPhase('perch'); }
          else if (phase === 'out') onDone();
        }}
      >
       <div className="nature-bird-scale" style={{ transform: `scale(${visit.scale})` }}>
...
export function NatureScene() {
  ...
  useEffect(() => {
    if (reduce) return;
    schedule(FAST ? 400 : rand(2000, 4500));
    return () => clearTimeout(timerRef.current);
  }, [reduce, schedule]);

  const handleDone = useCallback(() => {
    setVisit(null);
    schedule(FAST ? 1200 : rand(4500, 12000));
  }, [schedule]);
